...
- **Bumps the `@verevoir/sources` devDependency `^0.7.0` → `^0.9.0`** for the `/gitlab` subpath. The peer range stays `*`; consumers of `/gitlab` need sources ≥ 0.9.0. The full suite passes unchanged against 0.9.0, including sources 0.8.0's breaking fs change (a non-empty ref is refused) — nothing here passed a ref to the fs adapter.
